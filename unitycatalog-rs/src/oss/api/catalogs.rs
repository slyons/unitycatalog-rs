...
impl<'a> CatalogsClient<'a> {
    ...
    ) -> UCRSResult<ListCatalogResponse> {
        let mut url = self
            .client
            .base_url
            .clone()
            .join("/api/2.1/unity-catalog/catalogs")
    ...
    pub async fn get(&self, name: &str) -> UCRSResult<CatalogInfo> {
        let path = self
            .client
            .base_url
            .join(&format!("/api/2.1/unity-catalog/catalogs/{}", name))
            .map_err(UCRSError::MalformedURL)?;
        let res = self.client.get(path, None::<String>).await;
        if let Err(UCRSError::RequestError(ref res_inner)) = res {
            match res_inner.status() {
                Some(StatusCode::NOT_FOUND) => Err(UCRSError::CatalogNotFound(name.to_owned())),
                _ => res,
            }
        } else {
            res
    ...
    pub async fn update(&self, name: &str, update_props: UpdateCatalog) -> UCRSResult<CatalogInfo> {
        let path = self
            .client
            .base_url
            .join(&format!("/api/2.1/unity-catalog/catalogs/{}", name))
