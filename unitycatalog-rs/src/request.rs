...
impl RequestClient {
    pub fn new(base_url: &str, disable_ssl: bool) -> UCRSResult<Self> {
        let base_url = Url::parse(base_url).map_err(|e| UCRSError::MalformedURL(e))?;

        let client = Client::builder()
            .danger_accept_invalid_certs(disable_ssl)
            .build()
            .map_err(|e| UCRSError::ClientBuildError(e))?;
        ...
        R: DeserializeOwned,
    {
        ...
        let body = body
            .map(|b| serde_json::to_string(&b).map_err(|be| UCRSError::JSONFormattingError(be)));
